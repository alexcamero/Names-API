...
if LOCAL:
    DATA_DIR = path.join(BASE_DIR, '..', 'names_data')
    ...
    REMOTE_ENGINE = f"mysql+pymysql://{REMOTE_USER}:{REMOTE_PASSWORD}@{REMOTE_HOST}/{REMOTE_NAME}?ssl_ca={CA_PATH}"
else:
    ENGINE = f"mysql+pymysql://{DATABASE_USER}:{DATABASE_PASSWORD}@{DATABASE_HOST}/{DATABASE_NAME}?ssl_ca={CA_PATH}"
